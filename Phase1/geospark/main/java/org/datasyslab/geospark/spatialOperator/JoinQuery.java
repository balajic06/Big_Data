...
public class JoinQuery implements Serializable{
    ...
    public static JavaPairRDD<Envelope,HashSet<Point>> SpatialJoinQueryCartesian(JavaSparkContext sc, PointRDD objectRDD, RectangleRDD queryWindowRDD){

        List<Envelope> rectangles = queryWindowRDD.rawRectangleRDD.collect();

        List<Tuple2<Envelope,HashSet<Point>>> resultMap = new ArrayList<>();
        for (Envelope e: rectangles){

            PointRDD pointRDD = RangeQuery.SpatialRangeQuery(objectRDD,e,0);

            HashSet<Point> points = new HashSet<Point>(pointRDD.rawPointRDD.collect());

            resultMap.add(new Tuple2<>(e, points));
        }

        return aggregateJoinResultPointByRectangle(sc.parallelizePairs(resultMap));
    }
